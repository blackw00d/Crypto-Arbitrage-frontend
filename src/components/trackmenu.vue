  ...
  <div id="tracking_menu">
    ...
    <template v-for="(coins, coin_name) in listexchange">
      <select :id="coin_name" @change="selecting_track(coin_name)">
        ...
        <option v-for="coin in coins" :data-price="coin.price" :data-volume="coin.volume">
          {{ coin.name }}
        </option>
      </select>
    </template>
...
export default {
  ...
  methods: {
    ...
    selecting_track(item) {
      ...
      let volume = el.getAttribute('data-volume') ? el.getAttribute('data-volume') : 0

      this.list_tracking.push({
